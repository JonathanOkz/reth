    ...
    Ext: clap::Args + fmt::Debug,
{
        ...
        L: Launcher<C, Ext>,
    {
        ...
        snapshot::maybe_restore_snapshot(&snapshot, node_config.chain.chain_id(), &db_path, &static_files_path).await;

        tracing::info!(target: "reth::cli", path = ?db_path, "Opening database");
        let database = snapshot::open_db_with_optional_snapshot_recovery(
            &snapshot,
            db_path.clone(),
        ...
        if snapshot.snapshot_enabled {
            ...
            snapshot::spawn_backup_on_shutdown(
                task_executor,
                node_stopped_rx,
