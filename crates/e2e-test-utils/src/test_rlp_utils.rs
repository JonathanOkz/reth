...
pub fn generate_test_blocks(chain_spec: &ChainSpec, count: u64) -> Vec<SealedBlock> {
    ...
    for i in 1..=count {
        ...
        if chain_spec.is_london_active_at_block(header.number) {
            ...
            if let Some(parent_fee) = parent_base_fee {
                ...
                header.base_fee_per_gas = Some(alloy_eips::calc_next_block_base_fee(
                    ...
                    parent_gas_limit,
                    parent_fee,
                    chain_spec.base_fee_params_at_timestamp(reth_primitives::time::normalize_timestamp_to_seconds(header.timestamp)),
                ));
        ...
        if chain_spec.is_cancun_active_at_timestamp(reth_primitives::time::normalize_timestamp_to_seconds(header.timestamp)) {
