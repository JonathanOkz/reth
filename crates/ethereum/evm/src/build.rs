    ...
    ChainSpec: EthChainSpec + EthereumHardforks,
{
    ...
    fn assemble_block(
        ...
        // Convert milliseconds to seconds for chainspec/consensus checks
        ...
        if self.chain_spec.is_cancun_active_at_timestamp(ts_sec) {
            blob_gas_used =
                Some(transactions.iter().map(|tx| tx.blob_gas_used().unwrap_or_default()).sum());
            // Convert parent timestamp to seconds as well
            excess_blob_gas = if self.chain_spec.is_cancun_active_at_timestamp(parent.timestamp / 1_000) {
                parent.maybe_next_block_excess_blob_gas(
                    self.chain_spec.blob_params_at_timestamp(ts_sec),
                )
            } else {
