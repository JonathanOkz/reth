    ...
    F: FnOnce(BestTransactionsAttributes) -> BestTransactionsIter<Pool>,
{
    ...
    while let Some(pool_tx) = best_txs.next() {
        ...
        if let Some(blob_tx) = tx.as_eip4844() {
            ...
            let blob_sidecar_result = 'sidecar: {
                ...
                if chain_spec.is_osaka_active_at_timestamp(reth_primitives::time::normalize_timestamp_to_seconds(attributes.timestamp)) {
    ...
    let requests = chain_spec
        .is_prague_active_at_timestamp(reth_primitives::time::normalize_timestamp_to_seconds(attributes.timestamp))
