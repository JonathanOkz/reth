        ...
        async move {
            ...
            if let Some(fee_entries) = fee_entries {
                ...
                base_fee_per_gas.push(
                    self.provider()
                        .chain_spec()
                        .next_block_base_fee(&last_entry.header, last_entry.header.timestamp() / 1_000)
                ...
                for header in &headers {
                    ...
                    let blob_params = chain_spec
                        .blob_params_at_timestamp(header.timestamp() / 1_000)
                ...
                base_fee_per_gas.push(
                    chain_spec
                        .next_block_base_fee(last_header.header(), last_header.timestamp() / 1_000)
                        .unwrap_or_default() as u128,
                );
                ...
                base_fee_per_blob_gas.push(
                    ...
                    .maybe_next_block_blob_fee(
                        chain_spec.blob_params_at_timestamp(last_header.timestamp() / 1_000)
                    ).unwrap_or_default()
                );
            };
            ...
            self.provider()
                ...
                .and_then(|h| {
                    h.maybe_next_block_blob_fee(
                        self.provider().chain_spec().blob_params_at_timestamp(h.timestamp() / 1_000),
                    )
                })
