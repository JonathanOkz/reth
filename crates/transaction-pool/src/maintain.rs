    ...
    if let Ok(Some(latest)) = client.header_by_number_or_tag(BlockNumberOrTag::Latest) {
        ...
        let info = BlockInfo {
            ...
            pending_basefee: chain_spec
                .next_block_base_fee(latest.header(), reth_primitives::time::normalize_timestamp_to_seconds(latest.timestamp()))
                .unwrap_or_default(),
            pending_blob_fee: latest
                .maybe_next_block_blob_fee(chain_spec.blob_params_at_timestamp(reth_primitives::time::normalize_timestamp_to_seconds(latest.timestamp()))),
        };
        ...
        match event {
            CanonStateNotification::Reorg { old, new } => {
                ...
                let pending_block_base_fee = chain_spec
                    .next_block_base_fee(new_tip.header(), reth_primitives::time::normalize_timestamp_to_seconds(new_tip.timestamp()))
                    .unwrap_or_default();
                let pending_block_blob_fee = new_tip.header().maybe_next_block_blob_fee(
                    chain_spec.blob_params_at_timestamp(reth_primitives::time::normalize_timestamp_to_seconds(new_tip.timestamp())),
                );
            ...
            CanonStateNotification::Commit { new } => {
                ...
                let pending_block_base_fee = chain_spec
                    .next_block_base_fee(tip.header(), reth_primitives::time::normalize_timestamp_to_seconds(tip.timestamp()))
                    .unwrap_or_default();
                let pending_block_blob_fee = tip.header().maybe_next_block_blob_fee(
                    chain_spec.blob_params_at_timestamp(reth_primitives::time::normalize_timestamp_to_seconds(tip.timestamp())),
                );
